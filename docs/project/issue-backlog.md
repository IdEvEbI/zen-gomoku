...
- **当前优先级**：~~VCF~~ → ~~VCT（#70）~~ → ~~多威胁必防（#81）~~ → ~~VCT 题集（#83）~~ →  
  ...
  **冲四假 terminal（#93 · G）** → academy beginner 首着门禁 A（F/J/I）→ 中级/高级题 → 禁手 → 人设 #71 →  
...
| M1.2h | [#93](https://github.com/IdEvEbI/zen-gomoku/issues/93) | **fix: 冲四假 terminal（G）** | 挡后仅留叉的裸冲四不得抢软叉；048/076 回归；见 RESULTS §4.3                                      |
...
4. **下一刀建议**：[#93](https://github.com/IdEvEbI/zen-gomoku/issues/93) 模式 G 后，按 RESULTS 一类一 PR（F/J/I）；  
