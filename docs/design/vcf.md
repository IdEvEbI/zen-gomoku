...
2. **软威胁**（对方活三端等）→ 可能直接挡
3. **仅当无软威胁**：深层己方 VCF → 对方 VCF 必防
4. 否则 αβ

已知体验缺口：有时盘上已有己方 VCF，但对方活三触发软挡，根策略会**先挡活三**而不走 VCF。  
产品上可在 VCT 切片中调整优先级（「有强迫 VCF 优先于软挡」）；规格见 [vct.md](./vct.md)。
...
| 主题      | 说明                                                           |
| --------- | -------------------------------------------------------------- |
| 根优先级  | 己方 VCF 优先于软活三挡 → 并入 [vct.md](./vct.md) / #70 根策略 |
| 更深/更快 | 置换表增强、迭代加深、Worker                                   |
| **VCT**   | 设计草案 [vct.md](./vct.md)；实现 #70（仅唐僧）                |
