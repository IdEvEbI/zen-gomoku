...
>
> - **触发**：无硬必防；对方仅有软叉（尚无 ≥2 活四端逼应）；己方候选能落下后使 `findOpenFourMoves(self)` **≥2**（已强制双活四苗 / 双活四端，对方本手必须应其一）。
...
> - **不回退**：K（`c7`）、L（勿 `j9`）、M（`j8`）、G/J/I/F、j11。
...
| **双活四软挤（N）** | 契约已合入 [#113](https://github.com/IdEvEbI/zen-gomoku/pull/113)；**下一刀实现**（#6 → `h10`/`f8`）。参考中后盘反威胁被拆：`tang-human-of-counter-snuff-2026-08-11-10-46-01` |
