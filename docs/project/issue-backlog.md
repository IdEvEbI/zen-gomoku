...
- **当前优先级**：~~VCF~~ → ~~VCT（#70）~~ → ~~多威胁必防（#81）~~ → ~~VCT 题集（#83）~~ →  
  ...
  **先破对方短杀（#105 · L）** → academy beginner 首着门禁 A / 剩余 F（048/058）→  
  中级/高级题 → 禁手 → 人设 #71 →  
...
| M1.2n | [#105](https://github.com/IdEvEbI/zen-gomoku/issues/105) | **fix: 先破对方短杀（L）**        | 过早双活三造型；playtest `07-53-01`；契约文档先行                                                |
